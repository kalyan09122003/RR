...
export default function Header() {
  ...
  return (
    <>
            ...
            <nav className="hidden lg:flex items-center gap-8">
              {menuItems.map((item) => (
                <Link
                  key={item.href}
                  to={item.href}
                  className={`relative text-xs tracking-wider transition-all ${
                    isActive(item.href)
                ...
                >
                  ...
                  <span
                    className={`absolute left-0 -bottom-1 h-[2px] bg-[#00D4FF] transition-all ${
      ...
      <aside
        ref={drawerRef}
        className={`fixed top-0 right-0 z-50 h-full w-1/2 min-w-[280px] bg-[#0A0A0F]/95 backdrop-blur-2xl border-l border-white/10 transition-transform duration-300 ${
        ...
        <nav className="flex flex-col gap-2 pt-24 px-6">
          {menuItems.map((item) => (
            <Link
              ...
              to={item.href}
              onClick={() => setIsOpen(false)}
              className={`py-4 px-4 rounded-lg text-base ${
                isActive(item.href)
